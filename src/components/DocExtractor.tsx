...
export default function DocExtractor({ onAddScene, onAddNote, onDirectToPractice }: DocExtractorProps) {
  ...
  const [activeTab, setActiveTab] = useState<"scene" | "mindmap" | "drygoods">("scene");
  ...
  const handleExtract = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Extractor server encountered an error.");
      }
      ...
      alert("智能文档解析失败，正在为您启用本地高精算法进行模拟兜底以继续练习！");
  ...
  const handleConfirmClassification = () => {
    ...
    // Incorporate selected extensions into primary expressions
    const confirmedExpressions = [...extractedData.expressions];
    
    if (extractedData.suggestedExtensions) {
      extractedData.suggestedExtensions.forEach((ext, idx) => {
        if (selectedExtensions[idx] !== false) {
          confirmedExpressions.push(ext);
        }
      });
    }
  ...
  const handleSaveToCorpus = () => {
    ...
    // 2. Create and add individual Notes
    ...
    <div className="w-full max-w-4xl mx-auto px-4 py-8" id="doc-extractor-root">
      ...
      <div className="border-b border-neutral-200/50 pb-6 mb-8">
        ...
        <p className="text-xs text-neutral-400 mt-1 leading-normal">
          上传您个人的 Word (.docx)、PDF、或者笔记截图/截图图片。智能识别文字，按照口语情景归类、延展同境词并精炼思维导图与干货
        </p>
      </div>
                ...
                <div className="min-h-[280px]">
                  ...
                  {activeTab === "scene" && (
                    <div className="space-y-4">
                      ...
                      {/* Extended Expressions List */}
                      <div className="space-y-3.5">
                        <span className="text-[10px] font-bold text-neutral-400 font-mono block tracking-wider uppercase">
                          💎 场景配套原生词伙与延展词伙 ({extractedData.expressions.length})
                        </span>
